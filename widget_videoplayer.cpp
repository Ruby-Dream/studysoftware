...
widget_videoplayer::widget_videoplayer(QString videofile,QSqlDatabase db,QWidget *parent)
    : QWidget(parent)
    , ui(new Ui::widget_videoplayer)
{
    ui->setupUi(this);
    filename=videofile;//传入的文件名，绝对路径
    this->db=db;
...
void widget_videoplayer::on_bt_control_clicked()
{
...
void widget_videoplayer::on_bt_isloop_clicked(bool checked)
{
    loop=checked;
}

void widget_videoplayer::do_statechanged(QMediaPlayer::PlaybackState state)
{
...
void widget_videoplayer::on_volumeslider_valueChanged(int value)//拖动音量滑块条以设置音量
{
...
void widget_videoplayer::do_positionchanged(qint64 position)//音乐播放时，动态移动播放进度条，参数单位是毫秒
{
...
void widget_videoplayer::do_durationchanged(qint64 duration)//设置播放进度条最大值
{
...
void widget_videoplayer::on_playerslider_sliderReleased()//只有鼠标松开滑块时才改变音频播放位置，不能用valuechanged信号
{
    qint64 now=ui->playerslider->value();
    player->setPosition(now);
}

void widget_videoplayer::on_bt_volume_up_clicked()//音量加，然后触发on_volumeslider_valueChanged
{
    ui->volumeslider->setValue(ui->volumeslider->value()+5);
}


void widget_videoplayer::on_bt_volume_down_clicked()//音量减，然后触发on_volumeslider_valueChanged
{
    ui->volumeslider->setValue(ui->volumeslider->value()-5);
}


void widget_videoplayer::on_playerslider_sliderMoved(int position)//拖动进度条时动态变化时间戳
{
    QString playedtime=getstringtime(position);
    ui->playedtime->setText(playedtime);
}
...
void widget_videoplayer::on_timeview_doubleClicked(const QModelIndex &index)//双击时间戳时跳转播放位置
{
...
QString widget_videoplayer::getstringtime(int position)//将毫秒转换为mm:ss格式的字符串
{
...
int widget_videoplayer::getinttime(QString time)
{
    return time.mid(0,2).toInt()*60000+time.mid(3,2).toInt()*1000;
}

void widget_videoplayer::loadtimestamp()//加载该文件对应的时间节点和备注，根据时间升序排序
{
...
void widget_videoplayer::on_bt_newtime_clicked()//新建时间节点
{
...
void widget_videoplayer::on_bt_deletetime_clicked()//删除某个时间节点时
{
...
void widget_videoplayer::on_timeview_clicked(const QModelIndex &index)//单击时间节点项时
{
...
void widget_videoplayer::on_bt_save_clicked()//点击保存备注
{
    QSqlQuery query(db);
